...
- Simplified extension to a pure declarative theme — removed the Neon Dreams (CSS injection) and Reactor Glow runtime features, all commands, settings, and the TypeScript build system. The VSIX now ships only the theme JSON files, assets, and documentation.
- Replaced retired shields.io marketplace badges in README with GitHub-based badges (version from `package.json`, stars, license).
